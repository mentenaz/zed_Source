...
fn build_menus(title: impl Into<SharedString>, cx: &App) -> Vec<Menu> {
    vec![
        ...
        Menu {
            name: "Go".into(),
            items: vec![MenuItem::action("Go to...", OpenCommandPalette)],
            disabled: false,
        },
        Menu {
            name: "Theme".into(),
            items: vec![MenuItem::action("Select Theme...", SelectTheme)],
            disabled: false,
        },
        Menu {
            name: "Help".into(),
...
mod tests {
    ...
    fn build_menus_puts_navigation_actions_in_go_menu(cx: &mut TestAppContext) {
        ...
        cx.read(|cx| {
            ...
            assert_eq!(names, ["Story", "Edit", "Go", "Theme", "Help"]);
            ...
            let theme_menu = menus.iter().find(|menu| menu.name == "Theme").unwrap();
            assert_action(&theme_menu.items[0], "Select Theme...", |action| {
