...
mod themes;
mod title_bar;
/// Public so a host application's own menu (e.g. `forge_gpui`'s
/// `ForgeTitleBar`) can dispatch the same "open the theme command palette"
/// action `StoryRoot::on_action_select_theme` already handles, instead of
/// reimplementing that palette (live-preview-on-highlight, cancel-restores-
/// previous-theme, …) itself.
pub use crate::themes::SelectTheme;
...
/// The workspace root that a host application (e.g. `forge_gpui`) sets so
/// package-manager stories know which directory to operate on. Set via
/// `cx.set_global(ForgeWorkspaceRoot("H:/my-project".into()))` whenever the
/// workspace root changes; cleared to an empty string when no folder is open.
#[derive(Clone, Default)]
pub struct ForgeWorkspaceRoot(pub SharedString);
impl Global for ForgeWorkspaceRoot {}
    ...
    F: FnOnce(&mut Window, &mut App) -> E + Send + 'static,
{
    ...
    create_new_window_internal(
        title,
        window_size,
        move |window, cx, _view: &E| cx.new(|cx| AppTitleBar::new(title_owned, window, cx)).into(),
        crate_view_fn,
        cx,
    );
}

/// Like [`create_new_window`], but lets the host supply its own title bar
/// view (`title_bar_fn`) instead of the Gallery's own [`AppTitleBar`] — e.g.
/// a host application that wants full control of the window's top strip
/// (its own app menu, its own tabs, …) while keeping `StoryRoot`'s command
/// palette / notification / dialog infrastructure underneath it.
///
/// `title_bar_fn` receives a `&E` reference to the already-built main view
/// (before it's erased to `AnyView`), so the title bar can pull in state
/// that view owns — e.g. sharing a `Gallery` entity the main view created,
/// so the title bar can reflect its currently-active story.
pub fn create_new_window_with_title_bar<F, E, TBF, TB>(
    title: &str,
    title_bar_fn: TBF,
    crate_view_fn: F,
    cx: &mut App,
    ...
    TB: Into<AnyView>,
    F: FnOnce(&mut Window, &mut App) -> E + Send + 'static,
    TBF: FnOnce(&mut Window, &mut App, &E) -> TB + Send + 'static,
{
    create_new_window_internal(
        title,
        None,
        move |window, cx, view: &E| title_bar_fn(window, cx, view).into(),
        crate_view_fn,
        cx,
    );
}

fn create_new_window_internal<F, E>(
    title: &str,
    window_size: Option<Size<Pixels>>,
    title_bar_fn: impl FnOnce(&mut Window, &mut App, &E) -> AnyView + Send + 'static,
    crate_view_fn: F,
    cx: &mut App,
) where
    E: Into<AnyView>,
    ...
    cx.spawn(async move |cx| {
        ...
        let window = cx
            .open_window(options, |window, cx| {
                ...
                let title_bar = title_bar_fn(window, cx, &view);
                let story_root =
                    cx.new(|cx| StoryRoot::with_title_bar(title_bar, view, false, window, cx));

                // Set focus to the StoryRoot to enable it's actions. Unconditional
                // (not gated on `window.focused(cx).is_none()`): by the time this
                // deferred callback runs, `Root` (the window's outermost wrapper,
                // an ancestor of `StoryRoot`, not a descendant) has already claimed
                // default focus for itself. Since action dispatch only bubbles
                // upward from the focused node to the window root, focus sitting
                // on `Root` — above `StoryRoot` in the tree — means dispatch skips
                // straight past `StoryRoot`'s own `.on_action(...)` handlers (for
                // `SelectTheme`, `OpenCommandPalette`, etc.) and falls through to
                // the global fallback, so those actions silently never run. This
                // must unconditionally reclaim focus for `StoryRoot` every time.
...
impl StoryContainer {
    ...
    /// The type-erased view of the story this container wraps, if any —
    /// added so a host application can `AnyView::downcast` back to the
    /// concrete story type (see `Gallery::open_file`, which does this for
    /// `EditorStory`).
    pub fn story_view(&self) -> Option<AnyView> {
        self.story.clone()
    }
}
...
impl StoryRoot {
    pub fn new(
        ...
        let title_bar = cx.new(|cx| AppTitleBar::new(title, window, cx)).into();
    ...
    pub fn embedded(view: impl Into<AnyView>, window: &mut Window, cx: &mut Context<Self>) -> Self {
        let title_bar = cx.new(|cx| AppTitleBar::new("", window, cx)).into();

        Self::with_title_bar(title_bar, view, true, window, cx)
    }

    pub(crate) fn with_title_bar(
        title_bar: AnyView,
        view: impl Into<AnyView>,
        embedded: bool,
        window: &mut Window,
