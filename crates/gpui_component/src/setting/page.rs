...
pub struct SettingPage {
    ...
    pub(super) sidebar_suffix: Option<Rc<dyn Fn(&mut Window, &mut App) -> AnyElement>>,
...
impl SettingPage {
    ...
    /// Set a custom element to render after the title in the sidebar menu item.
    ///
    /// For example, a count badge next to the page's nav label.
    pub fn sidebar_badge<F, E>(mut self, badge: F) -> Self
    where
        E: IntoElement,
        F: Fn(&mut Window, &mut App) -> E + 'static,
    {
        self.sidebar_suffix = Some(Rc::new(move |window, cx| {
            badge(window, cx).into_any_element()
        }));
        self
    }
