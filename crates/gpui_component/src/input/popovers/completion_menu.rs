...
impl CompletionMenu {
    ...
    fn select_item(&mut self, item: &CompletionItem, window: &mut Window, cx: &mut Context<Self>) {
        let item = item.clone();
        eprintln!(
            "[compl-debug] select_item: label={:?} trigger_start_offset={:?} self.offset={}",
            item.label, self.trigger_start_offset, self.offset
        );
    ...
    pub(crate) fn handle_action(
        ...
        if input::Enter::is_primary(&*action) {
            ...
            // Not one of the keys the completion menu owns — let it
            // continue on to the editor's own handling (the default
            // dispatch behavior is to stop once *any* handler runs, so
            // this has to opt back in explicitly). Enter/Escape/Up/Down
            // above must NOT do this: those are fully consumed by the
            // menu (accept/dismiss/navigate) — letting them propagate too
            // was the bug (Enter both accepted the completion *and* fell
            // through to the editor's default "insert a newline" handling,
            // splitting the line the completion was just inserted into).
            cx.propagate();
            return false;
        }
