...
impl CodeActionMenu {
    ...
    pub(crate) fn handle_action(
        ...
        if input::Enter::is_primary(&*action) {
            ...
            // See the identical fix in `completion_menu.rs`'s
            // `handle_action` for why this only propagates in the
            // unhandled case — the same duplicate-effect bug (accepting
            // here *and* falling through to the editor's own handling of
            // the same key) applies here too, even though code actions
            // aren't wired to any real LSP provider yet.
            cx.propagate();
            return false;
        }
