...
// `pub`, not `pub(crate)`: a host application wiring its own diagnostics
// into the editor's decoration API (e.g. squiggly underlines) needs the
// same severity→color mapping the built-in diagnostic popover already uses,
// rather than re-deriving its own.
pub use diagnostic_styles::*;
