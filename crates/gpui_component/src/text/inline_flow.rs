...
fn measure_image_size(
    ...
    let intrinsic_size = if width.is_some() && height.is_some() {
        ...
        intrinsic_image_size(url, window, cx)
    };
    image_size(width, height, intrinsic_size, line_height, rem_size)
}

/// Reads the real, decoded pixel size of `url` straight from the image
/// asset cache instead of laying out a throwaway `img()` element.
///
/// The old approach called `layout_as_root` on a disconnected element tree,
/// which can't reliably report an image's natural dimensions before (or even
/// after) the async fetch completes: the element has no stable identity tied
/// to the real `InlineFlow`, so a load that finishes later never triggers a
/// re-measure. That left remote images with no explicit size — e.g. markdown
/// badges — permanently stuck on the square fallback in `image_size`,
/// squashing wide badges into a distorted, unreadable box. Going through
/// `window.use_asset` ties the lookup (and its re-render-on-load) to the
/// actual view being rendered, so the size updates once the asset lands.
fn intrinsic_image_size(
    url: &SharedUri,
    window: &mut Window,
    cx: &mut App,
) -> Option<Size<Pixels>> {
    let resource = Resource::Uri(url.clone());
    let data = window.use_asset::<ImgResourceLoader>(&resource, cx)?.ok()?;
    let size = data.render_size(0);
