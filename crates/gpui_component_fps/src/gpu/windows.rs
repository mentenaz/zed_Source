...
impl Probe {
    pub(super) fn new() -> Option<Self> {
        ...
        let mut probe = Self {
            query,
            counter: PDH_HCOUNTER(std::ptr::null_mut()),
...
// SAFETY: `PDH_HQUERY`/`PDH_HCOUNTER` are opaque PDH handles (process-local
// identifiers, not shared pointers into memory `Probe` doesn't own); PDH
// permits using a handle from any thread as long as it is not accessed
// concurrently, and `Probe` is moved into the sampling task rather than
// shared, so single-threaded access is upheld.
