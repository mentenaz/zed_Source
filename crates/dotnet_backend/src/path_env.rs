...
//! often misses user-installed tools (pyenv, nvm, scoop, cargo, .NET SDK, …),
//! since those only ever get added to the registry's `Environment` PATH value
...
//! HKCU/HKLM registry values so a child process like `dotnet` can
...
/// like `%DOTNET_ROOT(x86)%\dotnet` resolve against the environment before
/// being merged in. Reads the process environment — the same source
