...
impl<M: InputModeKind> TextElement<M> {
    ...
    fn highlight_lines(
        ...
        // No early return when there's no tree-sitter highlighter (e.g. a
        // language with no registered grammar, like ForgeFlow's `.fwrk`/
        // `.fdgn`): diagnostics and semantic tokens below are independent of
        // tree-sitter and must still get a chance to run, or a
        // grammar-less language never shows any color at all — even though
        // its LSP semantic tokens (see `semantic_token_styles` below) were
        // fetched successfully. Only the tree-sitter `styles()` call itself
        // is skipped per-range when there's no highlighter.
        ...
        let flush_range = |start_line: usize, end_line: usize, skip: bool, styles: &mut Vec<_>| {
            ...
            let range_styles = match (skip, highlighter.as_deref()) {
                (true, _) | (false, None) => vec![(byte_start..byte_end, HighlightStyle::default())],
                (false, Some(highlighter)) => highlighter.styles(
                    &(byte_start..byte_end),
