...
use gpui::{
    ...
    Styled as _, Subscription, UTF16Selection, Window, actions, div, point, prelude::FluentBuilder as _,
    px,
};
...
impl<M: InputModeKind> InputBaseState<M> {
    ...
    /// Real tree-sitter syntax highlight runs for an arbitrary byte `range`,
    /// independent of what's currently laid out/visible in the main
    /// viewport. For consumers that render their own windowed or
    /// full-document view of the same buffer (e.g. a code minimap) and need
    /// real colors rather than reimplementing highlighting.
    ///
    /// `None` outside [`LayoutMode::CodeEditor`] mode, or when no
    /// grammar/highlighter is registered yet (matches this file's own
    /// `element.rs` rendering path, which treats "no highlighter" as
    /// unstyled text rather than an error) — callers should fall back to
    /// rendering `range` with a single default style in that case.
    pub fn highlighted_runs(&self, range: Range<usize>) -> Option<Vec<(Range<usize>, HighlightStyle)>> {
        let highlighter = self.mode.highlighter()?;
        let highlighter = highlighter.borrow();
        let highlighter = highlighter.as_deref()?;
        Some(highlighter.styles(&range, self.editor_style.highlight_styles.as_ref()))
    }
