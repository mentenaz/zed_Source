...
/// LSP's own standard semantic-token-type vocabulary (`"class"`,
/// `"interface"`, `"struct"`, `"typeParameter"`, `"parameter"`,
/// `"enumMember"`, `"decorator"`, `"event"`, `"method"`, `"macro"`,
/// `"namespace"`, `"regexp"`, ...) does **not** line up one-to-one with the
/// tree-sitter capture vocabulary the theme actually has colors for
/// (confirmed live: every one of those names resolved to no style at all
/// until this fallback existed) — [`map_lsp_token_type`] translates a name
/// the theme doesn't recognize to the nearest capture name it does before
/// giving up (e.g. `"method"` → `"function"`, `"parameter"` → `"variable"`),
/// tried only as a fallback so a resolver that *does* understand the raw
/// LSP names directly still wins. `"keyword.modifier"`-style dotted names
/// still fall back to their prefix (`"keyword"`) as before. Because the
/// color is resolved from the name on every paint, theme switches recolor
/// semantic tokens with no provider cooperation. Token *modifiers* are
/// accepted but not currently mapped to styles.
///
    ...
    pub(crate) fn semantic_tokens_for_range(
        ...
        self.semantic_tokens[lo..hi]
            ...
            .filter_map(|(range, name)| {
                ...
                let style = theme
                    .style(name.as_ref())
                    .or_else(|| theme.style(map_lsp_token_type(name.as_ref())))?;
...
/// Translates a raw LSP semantic-token-type name to the nearest existing
/// tree-sitter capture name, for use only as a fallback when the theme
/// doesn't recognize the LSP name as-is (see
/// [`DocumentRangeSemanticTokensProvider`]'s doc comment for why this
/// exists at all). Best-effort, not a claim of semantic equivalence —
/// e.g. `"macro"` and `"method"` both landing on `"function"` just means
/// "closest available color", not "these are the same thing". Anything
/// already matching a real capture name (`"variable"`, `"function"`,
/// `"keyword"`, `"type"`, `"comment"`, `"string"`, `"number"`, `"operator"`,
/// `"enum"`, `"property"`, ...) passes through unchanged.
fn map_lsp_token_type(name: &str) -> &str {
    match name {
        "class" | "interface" | "struct" | "typeParameter" | "namespace" => "type",
        "parameter" => "variable",
...
fn decode_semantic_tokens(
    ...
    let mut out = Vec::with_capacity(tokens.data.len() / 5);
    let mut line: u32 = 0;
    let mut character: u32 = 0;

    // The wire format packs each token as 5 `u32`s: deltaLine, deltaStart,
    // length, tokenType, tokenModifiers. A trailing partial group (malformed
    // server data) is dropped rather than indexed out of bounds.
    for token in tokens.data.chunks_exact(5) {
        let &[delta_line, delta_start, length, token_type, _token_modifiers_bitset] = token
        else {
            continue;
        };
        ...
        let Some(name) = names.get(token_type as usize) else {
            continue;
        };
...
mod tests {
    ...
    fn test_decode_semantic_tokens_delta() {
        ...
        // Each group of 5: deltaLine, deltaStart, length, tokenType, tokenModifiers.
        let tokens = SemanticTokens {
            result_id: None,
            data: vec![0, 0, 4, 0, 0, 1, 2, 5, 1, 0],
        };
    ...
    fn test_decode_skips_out_of_legend_index() {
        let tokens = SemanticTokens {
            result_id: None,
            // token_type = 99 is out of legend bounds.
            data: vec![0, 0, 3, 99, 0],
        };
