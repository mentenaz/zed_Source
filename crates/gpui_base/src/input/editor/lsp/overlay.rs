...
impl InputBaseState<EditorMode> {
    ...
    pub fn insert_completion(
        ...
        eprintln!(
            "[compl-debug] insert_completion: label={:?} fallback_range={:?} doc_len={} has_text_edit={} insert_text={:?}",
            item.label,
            fallback_range,
            self.text.len(),
        ...
        eprintln!("[compl-debug] resolved byte range={:?} new_text={:?} doc_len={}", range, new_text, self.text.len());
        self.completion_inserting = true;
        let range = self.range_to_utf16(&range);
        eprintln!("[compl-debug] utf16 range={:?}", range);
        self.replace_text_in_range_silent(Some(range), &new_text, window, cx);
        self.completion_inserting = false;
        self.focus(window, cx);
        eprintln!("[compl-debug] after replace: doc_len={}", self.text.len());
    }
