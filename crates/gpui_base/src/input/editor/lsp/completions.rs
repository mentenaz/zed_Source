...
/// LSP 3.18 inline-completion types (`textDocument/inlineCompletion`).
///
/// The Zed fork of `lsp_types` this workspace pins predates the LSP 3.18
/// inline-completion extension, so these mirror its wire format locally
/// rather than depending on upstream `lsp_types`.
///
...
pub struct InlineCompletionTriggerKind(u8);
